...
> **Benchmark chart coming:** run `make bench` to generate `artifacts/cost_quality.png`, then re-enable the image line below.
<!-- ![Cost vs. quality](artifacts/cost_quality.png) -->
...
At runtime you can't peek at the correct answer to decide whether to escalate — so the escalation signal must be **answer-independent**. This repo uses **`<self-consistency | judge-model | logprob-confidence>`** ([ADR-0002](decisions/0002-cascade-routing-policy.md) explains the choice and the alternatives).
...
python -m router.demo "What is 17% of 240?"   # route a single prompt (runs with fakes, no keys)
...
- [ADR-0001 — Provider abstraction](decisions/0001-provider-abstraction.md)
- [ADR-0002 — Cascade routing & the escalation signal](decisions/0002-cascade-routing-policy.md)
- [ADR-0003 — Fallback & circuit-breaking](decisions/0003-fallback-and-circuit-breaking.md)
