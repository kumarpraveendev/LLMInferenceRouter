...
**Headline:** `<__>%` of frontier accuracy at `<__>%` lower cost. Methodology — a fixed public dataset (GSM8K), objective exact-match grading, an answer-independent escalation signal, and full cost accounting that includes any judge calls — is described under *How the escalation decision works* and *Honest limitations* below.
